...
/** Shared streaming decoder for UTF and legacy Cyrillic plain-text books. */
public final class TextStreamDecoder {
    private static final int SAMPLE_LIMIT = 16 * 1024;

    private TextStreamDecoder() { }
    ...
    private static Detected preferred(String preferredEncoding, byte[] sample) {
        ...
        if (preferredEncoding != null && !preferredEncoding.isBlank()) {
            try {
                return new Detected(Charset.forName(preferredEncoding.trim()), 0);
