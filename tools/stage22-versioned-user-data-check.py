...
need('while (current < CURRENT_SCHEMA_VERSION)' in adapter and 'migrateV1ToV2' in adapter, 'sequential manifest migration missing')
...
need('.restore.tmp' in service and 'ATOMIC_MOVE' in service, 'restore staging/atomic swap missing')
...
print(' - schema-v2 portable user-data sections + sequential v1 migration: PASS')
