...
/** Real FXMLLoader contract for the unified Reader tools sidebar. */
class ReaderWorkspaceSidebarFxTest {
    @BeforeAll
    ...
    @Test
    void readerWorkspaceLoadsUnifiedFiveTabSidebar() throws Exception {
        AtomicReference<Throwable> failure = new AtomicReference<>();
        AtomicReference<FXMLLoader> loaded = new AtomicReference<>();
        ...
        FXMLLoader loader = loaded.get();
        assertThat(loader).isNotNull();
        VBox sidebar = (VBox) loader.getNamespace().get("annotationSidebar");
        TabPane tabs = (TabPane) loader.getNamespace().get("readerSidebarTabs");
        ListView<?> annotations = (ListView<?>) loader.getNamespace().get("annotationSidebarList");
        ListView<?> bookMap = (ListView<?>) loader.getNamespace().get("bookMapSidebarList");
        assertThat(sidebar).isNotNull();
        assertThat(sidebar.isManaged()).isFalse();
        assertThat(tabs).isNotNull();
        assertThat(tabs.getTabs()).hasSize(5);
