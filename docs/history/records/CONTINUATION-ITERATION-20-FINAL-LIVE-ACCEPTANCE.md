...
Before finalization, the Windows evidence validator additionally rejects any report timestamp older than the current host/session binding and any unreferenced file inside the evidence directories. The nested Windows ZIP and final reviewer ZIP are bounded, traversal/symlink/encryption-safe and closed against unexpected members.
