...
public class SearchBooksUseCase {
    ...
    public List<BookDto> execute(String query, int limit) {
        if (query == null || query.isBlank()) {
            return bookQueryRepository.findAll(limit, 0).stream()
        ...
        if (ids.isEmpty()) {
            log.debug("Lucene не знайшов результатів, використовуємо SQL пошук за автором");
            return bookQueryRepository.searchByAuthor(query, limit).stream()
                    .map(bookMapper::toDto)
                    .collect(Collectors.toList());
        }
